...
/// Maps the slider's raw 0...1 drag position to a 1...500 word count using two
/// linear segments instead of one: the first half of the track covers 1-100 words
...
enum SliderCurve {
    ...
    static func wordCount(forPosition position: Double) -> Int {
        ...
        let count: Double
        if p <= 0.5 {
            count = Double(minCount) + (p / 0.5) * Double(breakpointCount - minCount)
        } else {
    ...
    static func position(forCount count: Int) -> Double {
        let c = Double(min(max(count, minCount), maxCount))
        if c <= Double(breakpointCount) {
            return (c - Double(minCount)) / Double(breakpointCount - minCount) * 0.5
        } else {
